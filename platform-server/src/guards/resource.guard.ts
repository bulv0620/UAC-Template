...
export class ResourceGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const everyone = this.reflector.get<boolean>('everyone', context.getHandler())
    ...
    // 从类级别获取资源名称
    const resourceName = this.reflector.get<string>('resource-name', context.getClass())
    if (resourceName) {
  ...
  private isResourceAccessible(resources: any[], resourceName: string): boolean {
